...
export const canRequestAgain = async (
  ...
  rideId?: string,
  tripId?: string
): Promise<{ canRequest: boolean; reason?: string; lastRejection?: Date }> => {
  try {
    let query = supabase
      .from('ride_confirmations')
      .select('*')
    ...
    // If rejected, check cooldown period (1 hour minimum between requests)
    if (latestConfirmation.status === 'rejected') {
      const lastRejection = new Date(latestConfirmation.updated_at)
      const hoursSinceRejection = (new Date().getTime() - lastRejection.getTime()) / (1000 * 60 * 60)
      
      if (hoursSinceRejection < 1) {
        return { 
          canRequest: false, 
          reason: 'Please wait at least 1 hour before requesting again',
          lastRejection
        }
      }
...
export const getReversalEligibility = async (
  ...
  try {
    ...
    if (!isOwner && !isPassenger) {
      return { canReverse: false, reason: 'You do not have permission to reverse this confirmation' }
    }
    ...
    // Check if ride/trip is still in the future
    const ride = confirmation.car_rides
    const trip = confirmation.trips
    const departureTime = ride ? new Date(ride.departure_date_time) : new Date(trip?.travel_date || '')
