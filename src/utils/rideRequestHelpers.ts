...
export const cleanupExpiredRequests = async (): Promise<{
  ...
  return retryWithBackoff(async () => {
    // Use the cleanup function from the notification service
    const { cleanupExpiredNotifications } = await import('./rideNotificationService')
    const result = await cleanupExpiredNotifications()

    return {
      expiredRequests: result.expiredRequests,
      expiredNotifications: result.expiredNotifications
    }
  })
}
