...
import { ArrowLeft, Calendar, Clock, DollarSign, Send, MapPin, Plus, X, Bell, Search, User } from 'lucide-react'
...
export default function PostRide({ onBack, isGuest = false }: PostRideProps) {
  ...
  const handleConfirmPost = async () => {
    ...
    try {
      ...
      // Create driver notification preference if enabled
      if (enableNotifications && data) {
        ...
        try {
          ...
          const notificationData = {
            ...
            departure_location: fromLocation.address,
            departure_latitude: fromLocation.latitude,
            departure_longitude: fromLocation.longitude,
            destination_location: toLocation.address,
            destination_latitude: toLocation.latitude,
            destination_longitude: toLocation.longitude,
            search_radius_miles: notificationRadius,
            date_type: notificationDateType,
            specific_date: notificationDateType === 'specific_date' ? notificationSpecificDate : undefined,
            multiple_dates: notificationDateType === 'multiple_dates' ? notificationMultipleDates.filter(d => d) : undefined,
            notification_month: notificationDateType === 'month' ? notificationMonth : undefined,
