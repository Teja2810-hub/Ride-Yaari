...
import { ArrowLeft, Calendar, MessageCircle, User, Car, TriangleAlert as AlertTriangle, Clock, DollarSign, ListFilter as Filter, Import as SortAsc, Dessert as SortDesc } from 'lucide-react'
...
export default function FindRide({ onBack, onStartChat, isGuest = false }: FindRideProps) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      console.log('=== FIND RIDE SEARCH DEBUG ===')
      ...
      console.log('Filtering rides after date:', now)
      ...
      if (fromLocation?.address) {
        console.log('Adding from location filter:', fromLocation.address)
        query = query.ilike('from_location', `%${fromLocation.address}%`)
      }
      
      if (toLocation?.address) {
        console.log('Adding to location filter:', toLocation.address)
        query = query.ilike('to_location', `%${toLocation.address}%`)
      }
      
      // Apply date filters
      if (searchByMonth && departureMonth) {
        console.log('Searching by month:', departureMonth)
        const startOfMonth = `${departureMonth}-01`
        const endOfMonth = new Date(departureMonth + '-01')
        endOfMonth.setMonth(endOfMonth.getMonth() + 1)
        const endOfMonthStr = endOfMonth.toISOString().split('T')[0]
        
        query = query
          .gte('departure_date_time', startOfMonth)
          .lt('departure_date_time', endOfMonthStr)
      } else if (!searchByMonth && departureDate) {
        console.log('Searching by specific date:', departureDate)
        const nextDay = new Date(departureDate)
        nextDay.setDate(nextDay.getDate() + 1)
        const nextDayStr = nextDay.toISOString().split('T')[0]
        
        query = query
          .gte('departure_date_time', departureDate)
          .lt('departure_date_time', nextDayStr)
      }
      ...
      if (!effectiveIsGuest && user) {
        console.log('Excluding user own rides for user:', user.id)
      ...
      console.log('Setting rides data:', data?.length || 0, 'rides found')
      setRides(data || [])
      setSearched(true)
    } catch (error: any) {
