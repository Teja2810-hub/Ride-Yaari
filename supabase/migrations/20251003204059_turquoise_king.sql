/*
  ...
  1. New Tables
    ...
    - `trips` - Airport trip postings
    - `car_rides` - Car ride postings with seat management
    - `ride_confirmations` - Ride confirmation requests
...
-- Add seat management columns to existing car_rides table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'car_rides' AND column_name = 'total_seats'
  ) THEN
    ALTER TABLE car_rides ADD COLUMN total_seats integer NOT NULL DEFAULT 4;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'car_rides' AND column_name = 'seats_available'
  ) THEN
    ALTER TABLE car_rides ADD COLUMN seats_available integer NOT NULL DEFAULT 4;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'car_rides' AND constraint_name = 'valid_seats'
  ) THEN
    ALTER TABLE car_rides ADD CONSTRAINT valid_seats CHECK (seats_available >= 0 AND seats_available <= total_seats);
  END IF;
END $$;
...
-- RLS Policies for ride_confirmations
...
-- Function to update seats_available when confirmations change
